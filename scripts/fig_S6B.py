"""
Script to visualize permutation test boxplots across networks and hemispheres for LH transfer early eccentricity and the transfer correlation (Figure S6B).

This script:
- Loads permutation test results from 'S6B_data.xlsx'.
- Plots boxplots for permutation null distributions and overlays observed correlation values for each hemisphere.
- Highlights statistical significance and formats axes consistently.
...
def plot_permute_bplots(data, fig_name='S6B_LH_transfer_permutaions.png', 
                        n_perm=1000, p_thresh=0.05):
    """
...
def main():
    ...
    data = pd.read_excel(os.path.join(DATA_DIR, 'S6B_data.xlsx'))
    
    plot_permute_bplots(data)
