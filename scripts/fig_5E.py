"""
Script to visualize the effect of task epoch on statistical maps and significant region eccentricity (Figure 5E).

This script:
- Loads epoch effect and region eccentricity data from '5E_data.xlsx'.
- Plots pairwise statistical maps for task epoch contrasts (early vs. baseline, late vs. early).
- Plots eccentricity measures for significant regions for each epoch comparison.
...
def plot_epoch_effect(data, prefix='5E_'):
    """
...
def plot_sig_region_eccentricity(ecc, fig_name):
    """
...
def main():
    ...
    data = pd.read_excel(os.path.join(DATA_DIR, '5E_data.xlsx'), sheet_name=None)

    plot_epoch_effect(data['epoch_effect_early_vs_baseline'])
    plot_epoch_effect(data['epoch_effect_late_vs_early'])

    plot_sig_region_eccentricity(data['early_vs_baseline_eccentricity'], 
                                 fig_name='5E_epoch_effect_early_vs_baseline_sig-regions_ecc.png')
    plot_sig_region_eccentricity(data['late_vs_early_eccentricity'], 
                                 fig_name='5E_epoch_effect_late_vs_early_sig-regions_ecc.png')
